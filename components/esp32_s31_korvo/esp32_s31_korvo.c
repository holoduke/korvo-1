...
/* Touch read with I2C bus supervision. The adapter ignores read errors, so a
 * touch controller that stops answering (NACK, SDA held low) would silently
 * kill input for good. Count consecutive failures and clock the bus free
 * (i2c_master_bus_reset issues the 9-clock recovery sequence + STOP). */
static uint32_t s_touch_recoveries;

static esp_err_t bsp_touch_read_supervised(esp_lcd_touch_handle_t tp, esp_lcd_touch_point_data_t *points,
                                           uint8_t *count, uint8_t max_count, void *user_ctx)
{
    ...
    if (err != ESP_OK) {
        fail_streak++;
        /* After 3 misses in a row (~100 ms) recover the bus; if it stays dead,
         * retry the recovery every ~3 s instead of every poll. */
        if (fail_streak == 3 || (fail_streak > 3 && (fail_streak % 100) == 0)) {
            esp_err_t r = s_i2c_bus ? i2c_master_bus_reset(s_i2c_bus) : ESP_ERR_INVALID_STATE;
            s_touch_recoveries++;
            ESP_LOGW(TAG, "touch I2C read failing (%s) -> bus reset %s (#%lu)", esp_err_to_name(err),
                     esp_err_to_name(r), (unsigned long)s_touch_recoveries);
        }
        *count = 0;
        return err;
    }
    if (fail_streak >= 3) {
        ESP_LOGI(TAG, "touch I2C recovered after %d failed reads", fail_streak);
    }
    fail_streak = 0;
    return esp_lcd_touch_get_data(tp, points, count, max_count);
}

uint32_t bsp_touch_get_recoveries(void)
{
    return s_touch_recoveries;
}
...
esp_err_t bsp_touch_new(esp_lv_adapter_rotation_t rotation, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    /* Finite transfer timeout. With the default (infinite) a NACK from the
     * GT1151 followed by a stuck-busy bus parks the LVGL task inside the I2C
     * driver forever (seen in the field: UI frozen, task watchdog on IDLE0). */
    touch_io_config.transaction_timeout_ms = 100;
...
lv_display_t *bsp_display_start_with_config(const bsp_display_config_t *config)
{
    ...
    if (!active_config.enable_touch) {
        ...
        if (ret != ESP_OK) {
            ...
            touch_config.callbacks.custom_touch_read = bsp_touch_read_supervised;
